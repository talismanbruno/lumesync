CORREÇÃO DE ESTADO E CSS: APLICAR AVATAR NO APP INTEIRO E AJUSTAR PREVIEW DO PERFIL

1. APLICAÇÃO GLOBAL DA FOTO/BANNER APÓS SALVAR (ATUALIZAÇÃO DE CACHE/ESTADO):
O avatar e o banner estão salvando no banco, mas o app inteiro continua mostrando as imagens velhas porque o estado global do perfil não é atualizado na hora do "Salvar Alterações".

Na função handleSave do SettingsModal.tsx, após fazer o UPDATE no Supabase:

OBRIGATÓRIO: Atualize o estado do contexto de autenticação/perfil imediatamente!

Se você usa um context ou zustand, chame a função de atualização (ex: updateProfile({ avatar_url: avatarPreview, banner_url: bannerPreview })).

Atualize também o profilesCache ou o array de usuários para que o chat e o Mini Profile Popover reflitam a mudança na mesma hora, sem precisar de F5.

Certifique-se de que a query inicial do chat (profiles(avatar_url)) e o componente da barra lateral leiam desse estado atualizado.

2. CORREÇÃO CSS DO PREVIEW DO PERFIL (SettingsModal.tsx):

O avatar está sobrepondo o nome de exibição na caixa de preview porque falta padding no container das informações.

Ajuste o container abaixo do banner no Preview para acomodar o avatar sobreposto com segurança:

codeTsx

<div className="relative w-full rounded-2xl bg-[#121214] border border-zinc-800 overflow-hidden mt-4">
  ...
  <div className="px-5 pb-5 pt-12 relative">
    ...
    <div className="flex items-center gap-1 mt-1">
      <h3 className="font-bold text-white text-lg">{name || 'Seu Nome'}</h3>
