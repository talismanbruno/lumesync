...
import { Hash, Plus, Search, Send, Volume2, UserPlus, Users, X, MessageSquare, Clock, Monitor, PhoneOff, Mic, MicOff, Headphones, Menu, Film, FileText, BadgeCheck, Settings2, ScreenShare } from "lucide-react";
...
function DashboardComponent() {
    ...
    useEffect(() => {
        ...
        if (globalProfile) {
            setDbProfile(globalProfile);
            setProfilesCache(prev => ({ ...prev, [globalProfile.id]: globalProfile }));
        }
    }, [authUser, globalProfile]);
    const myProfile = globalProfile ?? dbProfile ?? {
        id: authUser?.id || "",
    ...
    const [serverModalTab, setServerModalTab] = useState('create');
    ...
    const [activeDMGroup, setActiveDMGroup] = useState(null);
    ...
    const { participants, allParticipantsInRoom, screenStream, isMuted, isDeafened, isSharingScreen, remoteVideoStreams, peerConnections, toggleMute, toggleDeafen, toggleScreenShare, disconnect } = useVoiceRoom(activeVoiceChannel?.id || null, myProfile);
    ...
    const sendGif = async (url, gifData) => {
        if (!myProfile?.id)
            return;
        const messageData = {
            content: "",
    ...
    const handleCreateServer = async () => {
        if (!newServerName.trim() || !myProfile?.id)
            return;
        try {
    ...
    const handleJoinServer = async () => {
        if (!inviteCodeInput.trim() || !myProfile?.id)
            return;
        try {
    ...
    const handleDeleteServer = async () => {
        if (!serverToDelete || !myProfile?.id)
            return;
        try {
    ...
    const fetchFriendships = async () => {
        ...
        if (!error && friendshipsData) {
            const mapped = friendshipsData.map((f) => ({
                ...f,
    ...
    const handleSendFriendRequest = async () => {
        if (!addFriendUsername.trim() || !myProfile?.id)
            return;
    ...
    const handleSendMessage = async (e) => {
        if (e)
            e.preventDefault();
        if ((!newMessage.trim() && !selectedFile) || !myProfile?.id)
            return;
    ...
    useEffect(() => {
        if (!activeDMFriend || !myProfile?.id)
            return;
        markAsRead(activeDMFriend.id);
    ...
    return (<div className="flex h-full w-full overflow-hidden bg-transparent">
      ...
      {isMobileMenuOpen && (<div className="fixed inset-0 z-[60] bg-black/60 backdrop-blur-sm md:hidden" onClick={() => setIsMobileMenuOpen(false)}/>)}
        ...
        <div className="flex flex-col h-full w-full bg-[#0d0d10] border-r border-white/5">
          ...
          <div className="p-4 space-y-4">
            <div className="flex items-center justify-between">
              ...
              <button onClick={() => setIsSettingsOpen(true)} className="p-2 text-zinc-500 hover:text-white transition-colors md:hidden">
            ...
            <div className="relative group">
              ...
              <input type="text" placeholder="Busca rápida..." className="w-full bg-black/40 border border-white/5 rounded-xl py-2 pl-9 pr-4 text-xs text-white placeholder:text-zinc-600 focus:outline-none focus:border-cyan-500/30 transition-all"/>
            </div>
          </div>
          ...
          <div className="px-4 py-2">
            <div className="flex gap-1 p-1 bg-black/40 rounded-xl border border-white/5">
              <button onClick={() => { setActiveTab('conversas'); setActiveServer(null); }} className={`flex-1 flex items-center justify-center gap-1.5 py-2 text-[10px] font-bold uppercase tracking-wider rounded-lg transition-all ${activeTab === 'conversas' ? "bg-white/10 text-white shadow-sm" : "text-zinc-500 hover:text-zinc-300"}`}>
                <MessageSquare size={12}/>
                <span>Conversas</span>
              </button>
              <button onClick={() => { setActiveTab('servidores'); }} className={`flex-1 flex items-center justify-center gap-1.5 py-2 text-[10px] font-bold uppercase tracking-wider rounded-lg transition-all ${activeTab === 'servidores' ? "bg-white/10 text-white shadow-sm" : "text-zinc-500 hover:text-zinc-300"}`}>
                <Hash size={12}/>
                <span>Servidores</span>
              </button>
              <button onClick={() => { setActiveTab('amigos'); setActiveServer(null); setActiveDMFriend(null); }} className={`flex-1 flex items-center justify-center gap-1.5 py-2 text-[10px] font-bold uppercase tracking-wider rounded-lg transition-all ${activeTab === 'amigos' ? "bg-white/10 text-white shadow-sm" : "text-zinc-500 hover:text-zinc-300"}`}>
                <Users size={12}/>
                <span>Amigos</span>
              </button>
            </div>
          ...
          <div className="flex-1 overflow-y-auto overflow-x-hidden p-3 space-y-1 custom-scrollbar">
            {activeTab === 'conversas' && (<div className="space-y-1">
            ...
            }} className={`w-full flex items-center gap-3 px-2 py-2 rounded-xl transition-all ${activeDMFriend?.id === LUME_BOT_ID ? 'bg-white/5 text-white' : 'text-zinc-400 hover:bg-white/5 hover:text-zinc-200'}`}>
                ...
                if (!friend)
                    return null;
                return (<button key={friend.id} onClick={() => { setActiveDMFriend(friend); setActiveChannel(null); setShowVoiceUI(false); markAsRead(friend.id); }} className={`flex w-full items-center gap-3 rounded-xl px-2 py-2 text-sm transition-colors overflow-hidden ${activeDMFriend?.id === friend.id ? "bg-white/5 text-white" : "text-zinc-400 hover:bg-white/5 hover:text-zinc-200"}`}>
                        <div className="relative shrink-0">
                        ...
                        {(unreadCounts[friend.id] || 0) > 0 && (<span className="w-4 h-4 bg-cyan-500 text-black text-[10px] font-bold rounded-full flex items-center justify-center shrink-0 shadow-[0_0_10px_rgba(0,209,255,0.4)]">
            ...
            {activeTab === 'servidores' && (<div className="space-y-4">
                <div className="space-y-1">
                  ...
                  <div className="grid grid-cols-1 gap-1">
                    {servers.map(server => (<button key={server.id} onClick={() => setActiveServer(server)} className={`flex items-center gap-3 px-3 py-2 rounded-xl text-sm transition-all ${activeServer?.id === server.id ? "bg-white/10 text-white shadow-sm border border-white/5" : "text-zinc-400 hover:bg-white/5 hover:text-white"}`}>
                ...
                {activeServer && (<div className="space-y-4 animate-in slide-in-from-top-2 duration-300">
                    <div className="h-px bg-white/5 mx-2"/>
                    <div className="space-y-1">
                      <div className="flex items-center px-2 py-1 text-[10px] font-bold uppercase text-zinc-500 tracking-wider">Canais</div>
                      {channels.map(channel => (<button key={channel.id} onClick={() => {
                    ...
                    }} className={`flex w-full items-center gap-2.5 px-3 py-2 rounded-lg text-sm transition-colors ${(channel.type === 'text' && activeChannel?.id === channel.id) || (channel.type === 'voice' && activeVoiceChannel?.id === channel.id)
            ...
            {activeTab === 'amigos' && (<div className="space-y-4">
                <div className="space-y-1">
                  <div className="flex items-center px-2 py-2 text-[10px] font-bold uppercase text-zinc-500 tracking-wider">Amigos</div>
                  {['online', 'all', 'pending', 'add'].map(f => (<button key={f} onClick={() => setFriendFilter(f)} className={`flex w-full items-center gap-3 px-3 py-2 rounded-xl text-sm transition-colors ${friendFilter === f ? "bg-white/5 text-white" : "text-zinc-400 hover:bg-white/5 hover:text-zinc-200"}`}>
          ...
          <div className="p-4 bg-black/20 border-t border-white/5 space-y-3">
            ...
            {activeVoiceChannel && (<div className="bg-cyan-500/10 border border-cyan-500/20 rounded-2xl p-3 flex flex-col gap-2 animate-in fade-in slide-in-from-bottom-2">
                 <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-1">
                      <button onClick={() => setShowVoiceUI(true)} className="p-1 text-zinc-500 hover:text-white transition-colors">
                        <Monitor size={14}/>
                      </button>
                      <button onClick={() => {
                disconnect();
                ...
                <Popover open={showStatusMenu} onOpenChange={setShowStatusMenu}>
                  ...
                  <PopoverPortal>
                    <PopoverContent side="top" align="start" sideOffset={12} className="w-48 bg-[#0d0d11] border border-white/5 rounded-2xl p-1.5 shadow-2xl z-50 backdrop-blur-xl animate-in slide-in-from-bottom-2 duration-200">
                      {['online', 'idle', 'dnd', 'offline'].map((s) => (<button key={s} onClick={() => handleUpdateStatus(s)} className="w-full flex items-center gap-3 px-3 py-2.5 text-sm text-zinc-400 hover:bg-white/5 hover:text-white rounded-xl transition-colors text-left">
      ...
      <main className="flex-1 min-w-0 h-full bg-[#050505] flex flex-col relative overflow-hidden z-[50]">
        <div className="flex flex-1 flex-col overflow-hidden">
          {activeVoiceChannel && showVoiceUI ? (<VoiceRoomUI participants={participants} myProfile={myProfile} isMuted={isMuted} isDeafened={isDeafened} isSharingScreen={isSharingScreen} screenStream={screenStream} remoteVideoStreams={remoteVideoStreams} peerConnections={peerConnections} toggleMute={toggleMute} toggleDeafen={toggleDeafen} toggleScreenShare={toggleScreenShare} onDisconnect={() => {
                disconnect();
                setActiveVoiceChannel(null);
                setShowVoiceUI(false);
            }} onClose={() => setShowVoiceUI(false)}/>) : activeChannel || activeDMFriend ? (<>
              <header className="h-14 px-6 border-b border-white/5 flex items-center justify-between shrink-0 bg-transparent">
                ...
                {activeServer && (<button onClick={copyInvite} className="p-2 text-zinc-500 hover:text-white transition-colors flex items-center gap-2 text-xs font-bold uppercase tracking-widest">
                    <UserPlus size={14}/>
                    <span>Convidar</span>
                  </button>)}
              </header>
              ...
              <div className="flex-1 overflow-y-auto p-6 custom-scrollbar">
                <div className="max-w-4xl mx-auto space-y-6">
                  {messages.map((msg, idx) => (<div key={msg.id || idx} className="group flex items-start gap-4 animate-in fade-in slide-in-from-bottom-2 duration-300">
                      <UserProfileCard user={msg.profile || {}} isMe={msg.user_id === myProfile.id}>
                        <div className="shrink-0 cursor-pointer">
                          <UserAvatar avatarUrl={msg.profile?.avatar_url} name={msg.profile?.display_name || msg.profile?.username} size="h-10 w-10" className="rounded-xl border border-white/5 shadow-lg"/>
                        </div>
                      </UserProfileCard>
                      <div className="flex-1 min-w-0 space-y-1">
                        ...
                        {msg.file_url && (<div className="mt-2 rounded-2xl overflow-hidden border border-white/5 bg-black/20 max-w-sm">
                            {msg.file_type?.startsWith('image/') ? (<PhotoProvider>
                              ...
                              </PhotoProvider>) : msg.file_type?.startsWith('video/') ? (<video src={msg.file_url} controls className="w-full h-auto"/>) : (<div className="p-4 flex items-center gap-3">
              ...
              <div className="p-6 bg-gradient-to-t from-[#050505] to-transparent">
                <div className="max-w-4xl mx-auto relative group">
                  {attachmentPreview && (<div className="absolute bottom-full left-0 mb-4 p-2 bg-[#121214] border border-white/5 rounded-2xl animate-in slide-in-from-bottom-2">
                  ...
                  <form onSubmit={handleSendMessage} className="flex items-end gap-2 bg-[#121214] border border-white/5 rounded-2xl p-2 focus-within:border-cyan-500/30 transition-all shadow-2xl">
                    <button type="button" onClick={() => fileInputRef.current?.click()} className="p-2.5 text-zinc-500 hover:text-white transition-colors">
                      <Plus size={20}/>
                    </button>
            ...
            }} placeholder={isBotChat ? "Canal oficial (somente leitura)" : `Conversar em ${activeChannel ? '#' + activeChannel.name : '@' + (activeDMFriend?.display_name || activeDMFriend?.username)}`} disabled={isBotChat && !myProfile.is_admin} className="flex-1 bg-transparent border-none text-sm text-white py-2.5 px-2 resize-none max-h-32 focus:ring-0 placeholder:text-zinc-600 custom-scrollbar" rows={1}/>
                    <div className="flex items-center gap-1">
                      <button type="button" onClick={() => setShowGifPicker(!showGifPicker)} className={`p-2 transition-colors ${showGifPicker ? "text-cyan-400" : "text-zinc-500 hover:text-white"}`}>
                        <Film size={20}/>
                      </button>
                      <button type="submit" disabled={(!newMessage.trim() && !selectedFile) || isUploading} className="p-2.5 bg-cyan-500 text-black rounded-xl hover:bg-cyan-400 transition-all shadow-[0_0_15px_rgba(0,209,255,0.2)] disabled:opacity-50">
                        <Send size={18}/>
                      </button>
                    </div>
                  </form>
              ...
              <div className="flex gap-3">
                <Button onClick={() => setActiveTab('servidores')} variant="outline" className="rounded-xl border-white/5 hover:bg-white/5 text-zinc-400">
                  Explorar Servidores
                </Button>
                <Button onClick={() => setActiveTab('amigos')} className="bg-cyan-500 text-black hover:bg-cyan-400 rounded-xl font-bold px-6 shadow-[0_0_20px_rgba(0,209,255,0.1)]">
                  Adicionar Amigos
                </Button>
              </div>
            </div>)}
      ...
      <CreateGroupModal isOpen={isCreateGroupOpen} onClose={() => setIsCreateGroupOpen(false)} myProfile={myProfile} friendships={friendships} onCreateGroup={async (memberIds, name) => {
            try {
