...
export function VerifiedBadge({ size = 14, className = '', title = 'Administrador verificado do Lume' }: VerifiedBadgeProps) {
  return (
    <BadgeTooltip
      name={title === 'Administrador verificado do Lume' ? 'Administrador do Lume' : title}
      label={title}
      className={`inline-flex shrink-0 items-center justify-center text-accent-primary drop-shadow-[0_0_7px_rgba(123,230,255,0.35)] ${className}`}
    >
      <svg width={size} height={size} viewBox="0 0 20 20" fill="none" aria-hidden="true">
        ...
        <path d="m6.65 9.75 2.05 2.02 4.65-4.55" stroke="#090c1b" strokeWidth="1.75" strokeLinecap="round" strokeLinejoin="round" />
      </svg>
    </BadgeTooltip>
  );
}
